# Devboard TUI
This is a TUI wrapper for [devboard](https://github.com/MVN-14/devboard-go) to allow for easier terminal interactions

![demo](demo.gif)

Powered by [Charm tools](https://github.com/charmbracelet):
- [bubbletea](https://github.com/charmbracelet/bubbletea)/[bubbles](https://github.com/charmbracelet/bubbles) (TUI Framework)
- [lipgloss](https://github.com/charmbracelet/lipgloss) (Styling)
- [vhs](https://github.com/charmbracelet/vhs) (Demo gif)

## Installation

### With Go
```
git clone https://github.com/MVN-14/devboard-tui/
cd devboard-tui
go install devboard-tui.go
```

## Usage
**see *<a href="https://github.com/MVN-14/devboard-go" target="_blank">devboard</a>* for more info about usage and configuration**
